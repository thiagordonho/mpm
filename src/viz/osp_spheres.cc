...
OSPGeometry createGroundPlaneGeometry() {
  ...
  const ospcommon::vec4f stripeColor = ospcommon::vec4f{1.0f, 0.1f, 0.1f, 1.f};
  ...
  for (size_t i = 0; i < numStripes; i++) {
    ...
    vertices.push_back(
        Vertex{ospcommon::vec3f{-paddedExtent, yLevel, coord - stripeWidth}, up,
               stripeColor});
    vertices.push_back(
        Vertex{ospcommon::vec3f{paddedExtent, yLevel, coord - stripeWidth}, up,
               stripeColor});
    ...
    vertices.push_back(
        Vertex{ospcommon::vec3f{coord - stripeWidth, yLevel, -paddedExtent}, up,
               stripeColor});
    vertices.push_back(
        Vertex{ospcommon::vec3f{coord + stripeWidth, yLevel, -paddedExtent}, up,
               stripeColor});
...
void updateSpheresGeometry() {
  ...
  // geometry
  OSPData spheresData = ospNewData(g_spheres.size() * sizeof(Sphere), OSP_UCHAR,
                                   g_spheres.data());
...
int main(int argc, const char** argv) {
  ...
  if (initError != OSP_NO_ERROR) return initError;
